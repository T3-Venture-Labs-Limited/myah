...
from myah.constants import ERROR_MESSAGES, WEBHOOK_MESSAGES
...
from myah.utils.misc import parse_duration, validate_email_format
...
from myah.utils.webhook import post_webhook
from myah.utils.access_control import get_permissions, has_permission
from myah.utils.groups import apply_default_group_assignment
from myah.routers.containers import get_or_create_container

from myah.utils.redis import get_redis_client
from myah.utils.rate_limit import RateLimiter
...
async def _provision_container_background(user_id: str) -> None:
    """Fire-and-forget: pre-warm the user's agent container after signup."""
    try:
        await get_or_create_container(user_id)
        log.info(f'Container pre-provisioned for new user {user_id}')
    except Exception as exc:
        # Non-fatal — the container will be created on the user's first chat
        log.warning(f'Background container provisioning failed for {user_id}: {exc}')
