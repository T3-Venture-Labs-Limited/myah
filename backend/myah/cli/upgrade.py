...
instead of remembering the sequence:
  ...
  3. ``git -C <repo-root> pull`` — refreshes the Myah source. Skipped
     ...
     working tree is dirty (we never blow away unstaged work). Local
     platform image rebuilds are also skipped when git reports dirty or
     unknown source state, so ``upgrade`` does not deploy in-progress work.
...
def upgrade_command(
    ...
    _maybe_update_platform_image(
        repo_root,
        console=console,
        skip_local_build=git_pull_status in {'dirty', 'status_failed', 'pull_failed'},
    )
...
def _maybe_git_pull(repo_root: Path, *, console) -> str:  # noqa: ANN001
    ...
    if rc != 0:
        console.print(
            f'[yellow]⚠[/] `git pull` returned exit {rc} — '
            'skipping local platform rebuilds when applicable. '
