...
from myah.env import DATABASE_USER_ACTIVE_STATUS_UPDATE_INTERVAL

from myah.models.chats import Chats
from myah.models.groups import Groups, GroupMember

from myah.utils.misc import throttle
from myah.utils.validate import validate_profile_image_url
...
class UsersTable:
    ...
    def delete_user_by_id(self, id: str, db: Optional[Session] = None) -> bool:
        try:
            ...
            # ── Myah Composio: revoke cloud-side accounts and delete local session ──
            try:
                from myah.models.integration_session import IntegrationSessions
                from myah.utils.composio import revoke_all_for_user_sync
                IntegrationSessions.delete_by_user_id(id)
                revoke_all_for_user_sync(id)  # best-effort cloud-side revocation, SYNC client
            except Exception as e:
                logger.warning(f'composio cleanup failed for user={id}: {e}')
            # ────────────────────────────────────────────────────────────────────────
