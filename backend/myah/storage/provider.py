...
class S3StorageProvider(StorageProvider):
    ...
    def upload_file(self, file: BinaryIO, filename: str, tags: Dict[str, str]) -> Tuple[bytes, str]:
        ...
        # ── Myah: graceful fallback when S3 is misconfigured ─────────────
        # Uploads in Myah are dual-written: LocalStorageProvider ran first
        # and the file is already on disk. If S3 push fails (stale creds,
        # expired Supabase signature, network), don't break the user's
        # upload — return the local path. STORAGE_PROVIDER=local is the
        # documented Myah default; surfaced S3 failures hide that fact.
        # See e2e-output/report.md ISSUE-014.
        except Exception as e:
            log.warning(
                'S3 upload failed for %s (%s) — falling back to local storage path. '
