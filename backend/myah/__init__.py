...
def _build_uvicorn_kwargs(host: str, port: int, reload: bool) -> dict:
    ...
    effect of `serve` (importing myah.main triggers DB engine, Sentry init,
    etc.). Returns a dict that callers pass to uvicorn.run().

    Reads UVICORN_WORKERS from os.getenv directly (NOT from myah.env)
    to avoid pulling in env.py's module-load side effects. This keeps
    the helper truly pure for testing.
    """
...
# Migration for any caller of `myah dev`:
#   Before: myah dev [--host H] [--port P]
#   After:  myah serve --reload [--host H] [--port P]
# Production container's `CMD ["myah", "serve"]` is unaffected (no --reload
# by default).
