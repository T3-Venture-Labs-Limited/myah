...
class TestIntegrationsRoutes:
    ...
    def _setup_app(self):
        ...
        from myah.routers.integrations import router as integrations_router
    ...
    def test_list_integrations_returns_200(self):
        ...
        with patch('myah.routers.integrations.composio_utils') as mock_cu:
    ...
    def test_connect_toolkit_returns_redirect_url(self):
        ...
        """
        with patch('myah.routers.integrations.composio_utils') as mock_cu, patch(
            'myah.routers.integrations._sign_state', return_value='jwt-state'
        ):
    ...
    def test_callback_valid_state_redirects(self):
        ...
        with patch('myah.routers.integrations.WEBUI_SECRET_KEY', _SECRET):
            # Re-import _verify_state to pick up the patched secret.
            # Instead, we patch at the module level.
            from myah.routers import integrations as integ_mod
    ...
    def test_callback_logs_ca_id_owner_mismatch(self):
        ...
        integ_mod.WEBUI_SECRET_KEY = _SECRET
        try:
            with patch('myah.routers.integrations.composio_utils') as mock_cu, patch.object(
                integ_mod, 'logger'
            ) as mock_logger:
    ...
    def test_disconnect_returns_403_for_cross_tenant_ca_id(self):
        ...
        from myah.utils.composio import CrossTenantAccessError
...
class TestNameProtection:
    ...
    def _setup_app(self):
        ...
        from myah.routers.agent_config import router as agent_config_router
        from myah.utils.auth import get_verified_user
    ...
    def test_composio_name_returns_409(self):
        ...
        with patch('myah.routers.agent_config.web_call', new_callable=AsyncMock) as mock_wc:
    ...
    def test_other_names_pass_through(self):
        ...
        with patch('myah.routers.agent_config.web_call', new_callable=AsyncMock) as mock_wc, patch(
            'myah.routers.agent_config._ensure_feature_enabled'
        ):
...
class TestUserDeleteCleanup:
    ...
    def test_delete_user_removes_integration_session(self):
        ...
        with patch(
            'myah.models.integration_session.IntegrationSessions', mock_is
        ), patch(
            'myah.utils.composio.revoke_all_for_user_sync', return_value=0
        ) as mock_revoke, patch('myah.models.users.Chats') as mock_chats, patch(
            'myah.models.users.Groups'
        ) as mock_groups, patch(
            'myah.models.users.get_db_context'
        ) as mock_db_ctx:
