"""Tests for the _resolve_aux_default helper in myah.config.

Isolation note: This module uses a deferred import pattern to avoid caching
the real 'myah.config' in sys.modules at collection time. The
test_agent_config.py module (in a sibling directory) loads agent_config.py
via importlib with a stubbed 'myah.config'. If the real module is
...
def _isolate_openwebui_modules():
    """Clean myah.* from sys.modules before and after each test."""
    to_remove = [k for k in sys.modules if k.startswith('myah')]
...
def _get_resolver():
    import importlib
    mod = importlib.import_module('myah.config')
