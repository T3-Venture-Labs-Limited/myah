...
from myah.storage import provider
...
class TestS3StorageProvider:
    ...
    def test_upload_file(self, monkeypatch, tmp_path):
        upload_dir = mock_upload_dir(monkeypatch, tmp_path)
        # S3 checks
        # Myah: upload_file now falls back to local path when S3 is misconfigured
        # (stale creds, missing bucket, network failure) instead of raising —
        # the local copy was already written by LocalStorageProvider. See
        # e2e-output/report.md ISSUE-014.
        contents_fb, local_fb = self.Storage.upload_file(
            io.BytesIO(self.file_content), self.filename
        )
        assert contents_fb == self.file_content
        assert local_fb == str(upload_dir / self.filename)
        # Clean up the local file we just produced before running the happy path.
        (upload_dir / self.filename).unlink(missing_ok=True)
