...
def test_compositions_dict_has_all_six():
    from myah.utils.agui_compositions import COMPOSITIONS
...
def test_agui_types_exports_event_classes():
    ...
    from myah.utils.agui_types import (
        ToolCallStartEvent,
        ToolCallArgsEvent,
        ToolCallEndEvent,
        StateSnapshotEvent,
...
def test_agui_types_event_type_enum():
    from myah.utils.agui_types import EventType
...
def test_build_render_ui_snapshot_with_component_id():
    ...
    from myah.utils.agui_adapter import _build_render_ui_snapshot
...
def test_events_from_activity_text_terminal_command():
    from myah.utils.agui_adapter import events_from_activity_text
...
def test_extract_agui_events_from_stream_chunk_tool_calls_log():
    from myah.utils.agui_adapter import extract_agui_events_from_stream_chunk
...
def test_build_render_awareness_form_wizard():
    ...
    from myah.utils.chat_payload import _build_render_awareness
...
def test_composition_registry_register_and_get():
    ...
    from myah.utils.agui_compositions import CompositionRegistry
...
def test_reformat_action_for_agent_structured():
    ...
    from myah.utils.chat_payload import _reformat_action_for_agent
...
def test_wait_state_set_and_get():
    ...
    from myah.utils.chat_payload import set_agent_wait_state, get_agent_wait_state, clear_agent_wait_state
...
def test_events_from_output_items_logs_on_failure(caplog):
    ...
    from myah.utils.agui_adapter import events_from_output_items

    bad_items = [{'type': 'function_call', 'name': 'render_ui', 'call_id': 'c1', 'arguments': object()}]
    with caplog.at_level(logging.WARNING, logger='myah.utils.agui_adapter'):
...
def test_events_from_tool_event_logs_on_failure(caplog):
    ...
    from myah.utils.agui_adapter import events_from_tool_event
