...
def test_get_hermes_media_proxies_content():
    ...
    from myah.routers.hermes_media import get_hermes_media
    ...
    async def run():
        with patch('myah.routers.hermes_media.get_or_create_container',
                   return_value=mock_record), \
             patch('myah.routers.hermes_media._gateway_url',
                   return_value='http://localhost:8643/v1'), \
             patch('myah.routers.hermes_media.AGENT_BEARER_TOKEN', 'test-token'), \
...
def test_get_hermes_media_sets_auth_header():
    ...
    async def run():
        with patch('myah.routers.hermes_media.get_or_create_container',
             ...
             patch('myah.routers.hermes_media._gateway_url',
                   return_value='http://localhost:8643/v1'), \
             patch('myah.routers.hermes_media.AGENT_BEARER_TOKEN', 'secret-token'), \
             patch('httpx.AsyncClient') as mock_client_class:
...
def test_get_hermes_media_no_bearer_token_skips_auth_header():
    ...
    async def run():
        with patch('myah.routers.hermes_media.get_or_create_container',
             ...
             patch('myah.routers.hermes_media._gateway_url',
                   return_value='http://localhost:8643/v1'), \
             patch('myah.routers.hermes_media.AGENT_BEARER_TOKEN', ''), \
             patch('httpx.AsyncClient') as mock_client_class:
