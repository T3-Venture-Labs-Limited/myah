# Tests for the seed-on-first-spawn path in myah/routers/containers.py.
...
def _load_containers_module():
    """Load myah/routers/containers.py with all heavy deps stubbed."""
    ...
    honcho_mod = _make('myah.services.honcho', honcho_service=honcho_service_mock)
    ...
    telemetry_parent = _make('myah.utils.telemetry')
    ...
    spec = importlib.util.spec_from_file_location('myah.routers.containers', router_path)
...
async def test_container_first_spawn_seeds_aux_defaults(containers_mod):
    ...
    with (
        ...
        patch(
            'myah.routers.containers._start_container_sync',
            return_value=_make_start_info(adopt=False),
        ),
        patch(
            'myah.routers.containers._wait_for_ready',
...
class TestImageOverrideParser:
    ...
    def test_empty_string_returns_empty_dict(self):
        from myah.routers.containers import _parse_image_overrides
...
class TestImageOverrideAppliedAtSpawn:
    ...
    def test_override_passes_pinned_image_to_docker(self):
        ...
        with (
            patch(
                'myah.routers.containers._docker_client',
            ...
            patch('myah.routers.containers.Containers') as mock_containers_model,
            patch('myah.routers.containers.AGENT_BEARER_TOKEN', 'test-bearer'),
            patch('myah.routers.containers.AGENT_IMAGE', 'myah/agent:latest'),
            patch(
                'myah.routers.containers.AGENT_IMAGE_OVERRIDES',
