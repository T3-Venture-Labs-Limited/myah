...
from myah.env import REDIS_KEY_PREFIX
...
async def create_task(redis, coroutine, id=None):
    ...
    def _on_task_done(t: asyncio.Task):
        # Capture unhandled exceptions from background tasks so they
        # appear in Sentry instead of being silently discarded.
        # asyncio logs unhandled task exceptions to stderr but Sentry's
        # default asyncio integration does not always capture them.
        if not t.cancelled():
            exc = t.exception()
            if exc is not None:
                log.error('Background task %s failed: %s', task_id, exc, exc_info=exc)
                try:
                    import sentry_sdk
    ...
    # Add a done callback for cleanup and exception capture
