...
_LOCAL_DEV_BROWSER_HOSTS: tuple[str, ...] = (
    'localhost',
    '127.0.0.1',
)
...
def write_worktree_env(
    ...
    **H7 invariant (plan §2.2c):** the written file MUST contain
    ...
    """
    tailscale_ipv4s = _detect_tailscale_ipv4s()
    ...
    cors_allow_origin = _build_worktree_cors_allow_origin(
        frontend_port,
        extra_hosts=tailscale_ipv4s,
    )
    ...
    content = (
        ...
        f"export CORS_ALLOW_ORIGIN='{cors_allow_origin}'\n"
...
def _detect_tailscale_ipv4s() -> list[str]:
    """Return local Tailscale IPv4s for Mac-over-Tailscale browser QA.

    Worktree frontends bind to ``0.0.0.0`` so Zachary commonly tests them from
    his Mac at ``http://<ubuntu-tailscale-ip>:<frontend-port>``. Socket.IO does
    strict origin checks against ``CORS_ALLOW_ORIGIN``; if the generated
    worktree env only contains localhost, the app clears the splash screen and
    immediately churns on ``/ws/socket.io`` 400s. Detection is best-effort so
    machines without Tailscale keep working.
    """
    try:
        result = run(['tailscale', 'ip', '-4'], timeout=2)
    except Exception:
        return []
...
def _build_worktree_cors_allow_origin(
    frontend_port: int,
    *,
    extra_hosts: list[str] | tuple[str, ...] = (),
) -> str:
    """Build the semicolon-separated CORS origin list for a worktree frontend."""
    origins: list[str] = []
    for host in [*_LOCAL_DEV_BROWSER_HOSTS, *extra_hosts]:
        origin = f'http://{host}:{frontend_port}'
        if origin not in origins:
            origins.append(origin)

    # Keep the main checkout's default Vite port for compatibility with older
    # local browser sessions and existing smoke scripts.
    default_vite_origin = 'http://localhost:5173'
    if default_vite_origin not in origins:
        origins.append(default_vite_origin)
    return ';'.join(origins)
...
def write_worktree_platform_env(
    ...
    *,
    ...
    # True, and mirrors it into PUBLIC_DEPLOYMENT_MODE so SvelteKit's
    # $env/dynamic/public drives the OSS bootstrap path in dev.
    # Hosted leaves it unset (hosted is the implicit default).
    if mode == 'oss':
        set_env_var(target, 'MYAH_DEPLOYMENT_MODE', 'oss')
        set_env_var(target, 'PUBLIC_DEPLOYMENT_MODE', 'oss')
