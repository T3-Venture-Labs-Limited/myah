...
from myah.utils.misc import get_last_user_message, get_messages_content
...
def get_task_model_id(default_model_id: str, task_model: str, task_model_external: str, models) -> str:
    ...
    # The default_model_id may be a provider-catalog model (openrouter/openai/
    # anthropic/google model the user selected) that isn't in the platform's
    # admin-configured `models` registry. In that case treat it as external
    # and use the external task model if one is configured.
    default_entry = models.get(task_model_id)
    if default_entry and default_entry.get('connection_type') == 'local':
