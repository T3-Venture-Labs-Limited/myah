...
from myah.env import CHAT_STREAM_RESPONSE_CHUNK_MAX_BUFFER_SIZE
...
def convert_output_to_messages(output: list, raw: bool = False) -> list[dict]:
    ...
    for item in output:
        ...
        elif item_type == 'myah:code_interpreter':
        ...
        elif item_type.startswith('myah:'):
...
def stream_chunks_handler(stream: aiohttp.StreamReader):
    ...
    async def yield_safe_stream_chunks():
        ...
        async for data, _ in stream.iter_chunks():
            ...
            if skip_mode and max_buffer_size and len(buffer) > max_buffer_size:
                buffer = b''
            ...
            for i in range(len(lines) - 1):
                ...
                if skip_mode:
                    ...
                    if not max_buffer_size or len(line) <= max_buffer_size:
                        skip_mode = False
                        yield line
                    else:
                        yield b'data: {}'
                    ...
                    # Normal mode: check if line exceeds limit (skip size check when limit is None)
                    if max_buffer_size and len(line) > max_buffer_size:
                        skip_mode = True
                        yield b'data: {}'
                        yield b'\n'
            ...
            if not skip_mode and max_buffer_size and len(buffer) > max_buffer_size:
                skip_mode = True
