...
from open_webui.config import AUX_DEFAULT_TASKS, _resolve_aux_default
...
try:
    from open_webui.utils.telemetry.myah_metrics import record_container_startup as _record_container_startup
except Exception:
...
async def get_or_create_container(user_id: str, event_emitter=None) -> ContainerModel:
    ...
    """
    from open_webui.utils.hermes_web import is_oss_mode  # local import to avoid cycle
...
async def _get_or_create_container_locked(user_id: str, event_emitter=None) -> ContainerModel:
    ...
    from open_webui.services.honcho import honcho_service
...
def _build_catalog_map(catalog_data: dict) -> dict:
    ...
    from open_webui.utils.agent_proxy import normalize_catalog_models
...
async def _resolve_auxiliary_patch(user, provider: str) -> dict:
    ...
    """
    from open_webui.utils.hermes_web import web_call
