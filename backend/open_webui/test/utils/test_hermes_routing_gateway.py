...
async def test_resolve_gateway_port_returns_gateway_port_when_set():
    ...
    from open_webui.utils.hermes_web import _resolve_gateway_port

    record = MagicMock()
    record.gateway_port = 51234
    record.host_port = 51111

    with patch('open_webui.utils.hermes_web._ensure_container', new=AsyncMock(return_value=record)):
...
async def test_resolve_gateway_port_falls_back_to_host_port_when_null():
    ...
    try:
        ...
        with patch(
            'open_webui.utils.hermes_web._ensure_container', new=AsyncMock(return_value=record)
        ):
            port = await _resolve_gateway_port(_UserStub())
...
async def test_aux_call_routes_myah_v1_to_gateway_port():
    ...
    from open_webui.utils.agent_proxy import aux_call
    ...
    with patch('open_webui.utils.hermes_web._resolve_gateway_port',
               new=AsyncMock(return_value=18643)), \
         patch('open_webui.utils.hermes_web._resolve_chat_port',
               new=AsyncMock(return_value=18642)), \
         patch('open_webui.utils.agent_proxy.httpx.AsyncClient', _FakeClient), \
         patch('open_webui.routers.containers.AGENT_HOST', 'host.docker.internal'), \
         patch('open_webui.routers.containers.AGENT_BEARER_TOKEN', 'tok'):
...
def test_adopt_path_persists_gateway_port(monkeypatch):
    ...
    from open_webui.routers import containers as containers_router
