...
def test_collect_refs_finds_media_tag():
    from open_webui.utils.hermes_media_persist import _collect_refs_outside_code
...
def test_persist_and_rewrite_empty_text_returns_unchanged():
    from open_webui.utils.hermes_media_persist import persist_and_rewrite
...
def test_persist_and_rewrite_successful_media_tag():
    ...
    async def run():
        with (
            ...
            patch('open_webui.utils.hermes_media_persist.Storage') as mock_storage,
            patch('open_webui.utils.hermes_media_persist.Files') as mock_files,
        ):
...
def test_persist_and_rewrite_preserves_code_block():
    ...
    async def run():
        with patch(
            'open_webui.utils.hermes_media_persist._persist_ref',
...
def test_persist_and_rewrite_inserts_chat_file():
    ...
    async def run():
        with (
            ...
            patch('open_webui.utils.hermes_media_persist.Storage') as mock_storage,
            patch('open_webui.utils.hermes_media_persist.Files') as mock_files,
            patch('open_webui.utils.hermes_media_persist.Chats') as mock_chats,
        ):
            mock_client = AsyncMock()
            mock_client.get = AsyncMock(return_value=mock_response)
            mock_client.__aenter__ = AsyncMock(return_value=mock_client)
