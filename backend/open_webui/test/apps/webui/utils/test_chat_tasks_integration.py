...
The handler is loaded with importlib stubs for heavy deps:
  - open_webui.routers.tasks (aux_call patched here)
  - open_webui.socket.main
  - opentelemetry.*

Patch target: open_webui.routers.tasks.aux_call — this is the binding that
chat_tasks.py actually calls after its `from open_webui.routers.tasks import ...`.
...
# ---------------------------------------------------------------------------
# DB setup — must happen before any open_webui import so env.py sees the URL
# ---------------------------------------------------------------------------
...
def _load_chat_tasks_module(aux_call_mock: AsyncMock) -> ModuleType:
    """Load open_webui.utils.chat_tasks with all heavy deps stubbed out.
    ...
    for key in list(sys.modules.keys()):
        if key.startswith('open_webui.utils.chat_tasks') or key.startswith('open_webui.routers.tasks'):
            del sys.modules[key]

    # ── Stub open_webui.routers.tasks with our aux_call mock ──
    ...
    task_stubs = {
        'open_webui.internal.db': _make_mod('open_webui.internal.db', get_session=MagicMock()),
        'open_webui.config': _make_mod(
            'open_webui.config',
        ...
        'open_webui.utils.auth': _make_mod(
            'open_webui.utils.auth',
            get_verified_user=MagicMock(),
            get_admin_user=MagicMock(),
        ),
        'open_webui.utils.agent_proxy': _make_mod(
            'open_webui.utils.agent_proxy',
            aux_call=aux_call_mock,
        ),
        'open_webui.models.users': _make_mod('open_webui.models.users', Users=MagicMock(), UserModel=MagicMock()),
        'open_webui.models.models': _make_mod(
            'open_webui.models.models',
            Models=SimpleNamespace(get_all_models=MagicMock(return_value=[])),
        ),
        'open_webui.utils.task': _make_mod(
        ...
        'open_webui.utils.chat': _make_mod('open_webui.utils.chat', generate_chat_completion=AsyncMock()),
    }

    # Patch env/constants only if not already loaded (they may be real from DB setup)
    if 'open_webui.env' not in sys.modules:
        task_stubs['open_webui.env'] = _make_mod(
            'open_webui.env',
            WEBUI_AUTH=True,
    ...
    # Load tasks.py
    spec = importlib.util.spec_from_file_location('open_webui.routers.tasks', tasks_path)
    tasks_mod = importlib.util.module_from_spec(spec)
    sys.modules['open_webui.routers.tasks'] = tasks_mod
    try:
    ...
    sys.modules['open_webui.socket.main'] = socket_main_stub
    ...
    if 'open_webui.constants' not in sys.modules:
        sys.modules['open_webui.constants'] = _make_mod(
    ...
    if 'open_webui.utils.misc' not in sys.modules:
        sys.modules['open_webui.utils.misc'] = _make_mod(
    ...
    spec2 = importlib.util.spec_from_file_location('open_webui.utils.chat_tasks', chat_tasks_path)
    chat_tasks_mod = importlib.util.module_from_spec(spec2)
    sys.modules['open_webui.utils.chat_tasks'] = chat_tasks_mod
...
def db_session():
    ...
    from open_webui.models.chats import Chat  # noqa: F401 — registers table
