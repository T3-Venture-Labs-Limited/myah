...
from open_webui.utils import hermes_web
...
def _patch_record(monkeypatch, record: Optional[ContainerModel]):
    ...
    """
    monkeypatch.setattr(
        'open_webui.utils.hermes_web.Containers.get_by_user_id',
        lambda _user_id: record,
    )
    # Stub the cold-path spawn-or-restart helper. The patch target is the
    # function inside ``open_webui.routers.containers``; ``hermes_web`` does
    ...
    monkeypatch.setattr(
        'open_webui.routers.containers.get_or_create_container',
        _async_record,
    )
...
def _patch_client(monkeypatch, captured: dict, *, response: _FakeResponse):
    ...
    monkeypatch.setattr('open_webui.utils.hermes_web.httpx.AsyncClient', _FakeClient)
...
async def test_web_call_503_on_connect_error(monkeypatch):
    ...
    monkeypatch.setattr('open_webui.utils.hermes_web.httpx.AsyncClient', _BoomClient)
...
async def test_web_call_504_on_timeout(monkeypatch):
    ...
    monkeypatch.setattr('open_webui.utils.hermes_web.httpx.AsyncClient', _SlowClient)
...
async def test_web_call_emits_sentry_breadcrumb(monkeypatch):
    ...
    crumbs: list = []
    monkeypatch.setattr(
        'open_webui.utils.hermes_web.sentry_sdk.add_breadcrumb',
