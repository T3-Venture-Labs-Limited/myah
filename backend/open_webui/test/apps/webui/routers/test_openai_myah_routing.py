...
def test_empty_payload_returns_empty_list():
    from open_webui.routers.openai import _build_myah_attachments
...
def test_single_file_produces_single_attachment():
    ...
    with patch('open_webui.routers.openai.Files.get_file_by_id', return_value=fake_file):
        payload = {
...
def test_rejects_aggregate_oversize():
    ...
    with patch(
        'open_webui.routers.openai.Files.get_file_by_id',
...
def test_missing_file_skips_silently():
    ...
    user = MagicMock(id='user-1')
    with patch('open_webui.routers.openai.Files.get_file_by_id', return_value=None):
...
def test_file_id_from_url_extracts_id():
    from open_webui.routers.openai import _file_id_from_url
...
def test_extract_model_provider_returns_first_tag_name():
    from open_webui.routers.openai import _extract_model_provider
...
class TestAttachmentForwardingIntegration:
    ...
    def test_attachment_forwarding_logs_warning_on_build_failure(self):
        ...
        with patch('sentry_sdk.add_breadcrumb', side_effect=lambda **kw: breadcrumb_calls.append(kw)):
            ...
            router_log = _logging.getLogger('open_webui.routers.openai')
    ...
    def test_no_hermes_media_feature_flag_in_router_source(self):
        ...
        """
        import inspect
        import open_webui.routers.openai as _module
    ...
    def test_insert_chat_files_called_when_attachments_forwarded(self):
        ...
        with (
            patch('open_webui.routers.openai.Files.get_file_by_id', return_value=fake_file),
            patch(
                'open_webui.routers.openai.Chats.insert_chat_files', side_effect=lambda **kw: insert_calls.append(kw)
            ),
        ):
            ...
            if _attachments:
                ...
                if metadata.get('chat_id') and metadata.get('message_id'):
                    try:
                        from open_webui.routers.openai import Chats  # noqa: F401 — trigger import
                        import open_webui.routers.openai as _router
