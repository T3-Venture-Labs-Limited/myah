...
def test_agent_actor_sentinel_shape():
    ...
    from open_webui.routers.files import AgentActor
...
async def test_agent_bearer_returns_agent_actor_without_touching_user_auth():
    ...
    """
    from open_webui.routers.files import AgentActor, get_file_user_or_agent

    with patch('open_webui.routers.files.AGENT_BEARER_TOKEN', 'test-agent-token'), \
         patch(
             'open_webui.routers.files.get_current_user',
...
async def test_wrong_bearer_falls_through_to_user_auth():
    ...
    from open_webui.routers.files import get_file_user_or_agent

    fake_user = MagicMock(spec=UserModel)
    fake_user.role = 'user'

    with patch('open_webui.routers.files.AGENT_BEARER_TOKEN', 'correct-agent-token'), \
         patch(
             'open_webui.routers.files.get_current_user',
...
async def test_empty_agent_bearer_token_disables_agent_path():
    ...
    with patch('open_webui.routers.files.AGENT_BEARER_TOKEN', ''), \
         patch(
             'open_webui.routers.files.get_current_user',
             new=AsyncMock(return_value=fake_user),
         ):
...
async def test_no_bearer_header_falls_through_to_user_auth():
    ...
    with patch('open_webui.routers.files.AGENT_BEARER_TOKEN', 'secret'), \
         patch(
             'open_webui.routers.files.get_current_user',
             new=AsyncMock(return_value=fake_user),
         ):
