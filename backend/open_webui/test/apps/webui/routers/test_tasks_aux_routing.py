...
def _load_tasks_module():
    ...
    mods = {
        'open_webui.internal.db': _make('open_webui.internal.db', get_session=MagicMock()),
        'open_webui.config': _make(
            'open_webui.config',
        ...
        'open_webui.utils.auth': _make(
            'open_webui.utils.auth',
            get_verified_user=MagicMock(),
            get_admin_user=MagicMock(),
        ),
        'open_webui.utils.agent_proxy': _make(
            'open_webui.utils.agent_proxy',
            aux_call=aux_call_mock,
        ),
        'open_webui.models.users': _make('open_webui.models.users', Users=MagicMock(), UserModel=MagicMock()),
        'open_webui.models.models': _make(
            'open_webui.models.models', Models=SimpleNamespace(get_all_models=MagicMock(return_value=[]))
        ),
        'open_webui.utils.task': _make(
        ...
        'open_webui.utils.chat': _make('open_webui.utils.chat', generate_chat_completion=AsyncMock()),
    }
    ...
    spec = importlib.util.spec_from_file_location('open_webui.routers.tasks', router_path)
    module = importlib.util.module_from_spec(spec)
    sys.modules['open_webui.routers.tasks'] = module
    try:
    ...
    module._stub_keys = list(mods.keys()) + ['open_webui.routers.tasks']
    return module
...
def tasks_mod():
    ...
    modules need (e.g. `open_webui.internal.db.Base`,
    `open_webui.env.DEFAULT_GROUP_SHARE_PERMISSION`). We save and restore all
    open_webui.* entries that were in sys.modules before injection.
    ...
    conflicts.
    """
    # Snapshot all open_webui.* entries currently in sys.modules.
    _PREFIX = 'open_webui'
    ...
    _ORM_MODULES = {
        'open_webui.config',
        'open_webui.env',  # env defines no ORM models; safe to restore
    }
