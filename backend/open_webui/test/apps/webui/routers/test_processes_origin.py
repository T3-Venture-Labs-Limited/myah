...
def _make_chat_lookup(chat_obj=None):
    ...
    return patch(
        'open_webui.models.chats.Chats.get_chat_by_id_and_user_id',
...
async def test_create_process_with_chat_id_forwards_origin():
    ...
    from open_webui.routers.processes import create_process, ProcessCreateForm
    ...
    with patch('open_webui.routers.processes._hermes_post', side_effect=fake_hermes_post), \
         patch('open_webui.routers.processes._ensure_container', side_effect=fake_ensure_container), \
...
async def test_create_process_rejects_unknown_chat_id():
    ...
    with patch('open_webui.routers.processes._hermes_post', side_effect=fake_hermes_post) as mock_post, \
         patch('open_webui.routers.processes._ensure_container', side_effect=fake_ensure_container), \
