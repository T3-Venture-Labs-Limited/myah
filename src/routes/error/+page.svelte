			...
			<div class="m-auto pb-44 flex flex-col justify-center">
				<div class="max-w-md">
					<div class="text-center text-2xl font-medium z-50">
						{$i18n.t('{{webUIName}} Backend Required', { webUIName: $MYAH_NAME })}
					</div>

					<div class=" mt-4 text-center text-sm w-full">
						{$i18n.t(
							"Oops! You're using an unsupported method (frontend only). Please serve Myah from the backend."
						)}

						<br class=" " />
						<br class=" " />
						<span class=" font-medium underline">{$i18n.t('See readme.md for instructions')}</span>
						{$i18n.t('or')}
