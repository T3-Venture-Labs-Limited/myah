		...
		editor = new Editor({
			...
			extensions: [
				StarterKit.configure({
					...
					// Disable extensions that are superseded by CodeBlockLowlight and ListKit
					// when richText is on. Both of those kits register the same extension names
					// (codeBlock, bulletList, listItem, listKeymap, orderedList), so keeping
					// them in StarterKit causes Tiptap's duplicate-extension warning (T3-858).
					codeBlock: false,
					bulletList: false,
					listItem: false,
					listKeymap: false,
