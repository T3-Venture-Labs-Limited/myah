<script lang="ts">
	...
	import DeclarativeUI from '$lib/components/generative-ui/DeclarativeUI.svelte';
	...
	$: declarative = (() => {
		try {
			const raw = decode(attributes?.declarative ?? '');
			if (!raw) return null;
			const parsed = JSON.parse(raw);
			return typeof parsed === 'object' && parsed?.blocks ? parsed : null;
		} catch {
			return null;
		}
	})();
...
<div {id} class={className}>
	{#if declarative}
		<div class="py-1 w-full">
			<DeclarativeUI spec={declarative} messageId={id} on:ui-interaction />
		</div>
