	...
	<div slot="content">
		...
		>
			...
			{#if !$temporaryChatEnabled && chat?.id}
						...
						{#each $folders.sort((a, b) => b.updated_at - a.updated_at) as folder}
							{#if folder?.id}
								...
								>
									...
									<div class="truncate">{folder.name ?? 'Space'}</div>
								</button>
							{/if}
						{/each}
					</DropdownSub>
				...
				<button
					draggable="false"
					class="flex gap-2 items-center px-3 py-1.5 text-sm cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 rounded-xl select-none w-full text-red-500"
					on:click={() => {
						deleteChatHandler();
					}}
				>
