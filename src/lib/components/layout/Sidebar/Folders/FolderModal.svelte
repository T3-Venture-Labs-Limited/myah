<script lang="ts">
	...
	const submitHandler = async () => {
		...
		if (maxFileCount && (data?.files ?? []).length > maxFileCount) {
			toast.error(
				$i18n.t('Maximum number of files per space is {{max}}.', { max: maxFileCount ?? 0 })
			);
			loading = false;
			return;
		}
			...
			<div class=" flex flex-col w-full sm:flex-row sm:justify-center sm:space-x-6">
				...
				>
					<div class="flex flex-col w-full mt-1">
						...
						<div class="flex-1">
							<input
								...
								type="text"
								bind:value={name}
								placeholder={$i18n.t('Enter space name')}
								autocomplete="off"
							/>
						</div>
					</div>
					...
					<div class="flex justify-between w-full mt-1 items-center">
						<div class="text-xs text-gray-500">{$i18n.t('Space Background Image')}</div>
