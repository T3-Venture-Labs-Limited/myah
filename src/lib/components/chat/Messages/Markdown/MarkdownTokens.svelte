<script lang="ts">
	...
	// render_*/show_* tool calls that carry a declarative spec are rendered inline
	// by ToolCallDisplay as a DeclarativeUI block — they must NOT be collapsed into
	// the ConsecutiveDetailsGroup accordion or users can't see them without clicking.
	const isRenderToolWithDeclarative = (
		token: Token & { attributes?: { type?: string; name?: string; declarative?: string } }
	) => {
		const name = token?.attributes?.name ?? '';
		return (
			(name.startsWith('render_') || name.startsWith('show_')) && !!token?.attributes?.declarative
		);
	};
	...
	const isGroupableDetailToken = (
		...
		if (!token?.type || token.type !== 'details') return false;
		if (!GROUPABLE_DETAIL_TYPES.has(token?.attributes?.type ?? '')) return false;
		// Declarative render tools must render inline — never collapse them
		if (isRenderToolWithDeclarative(token)) return false;
		return true;
	};
