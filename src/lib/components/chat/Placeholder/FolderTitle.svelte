<script lang="ts">
	...
	const updateHandler = async ({ name, meta, data }) => {
		if (name === '') {
			toast.error($i18n.t('Space name cannot be empty.'));
			return;
		}
...
{#if folder}
	...
	>
		...
		<div class="flex items-center gap-1.5">
			...
			<div class="text-xs text-gray-500">
				{$i18n.t('Delete all contents inside this space')}
			</div>
		</div>
	</DeleteConfirmDialog>
	...
	<div class="mb-3 px-6 @md:max-w-3xl justify-between w-full flex relative group items-center">
		<div class="text-center flex gap-3.5 items-center">
			...
			>
				<button
					aria-label={$i18n.t('Change space icon')}
		...
		<div class="flex items-center translate-x-2.5">
			...
			>
				<button
					...
					aria-label={$i18n.t('Space options')}
					on:click={(e) => {}}
				>
