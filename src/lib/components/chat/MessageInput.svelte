<script lang="ts">
	...
	import SlashCommandTray from './MessageInput/SlashCommandTray.svelte';
	...
	// Client-side pre-flight size limits
	const MAX_PER_FILE_BYTES = 20 * 1024 * 1024; // 20 MB
	const MAX_AGGREGATE_BYTES = 80 * 1024 * 1024; // 80 MB
	...
	const inputVariableHandler = async (text: string): Promise<string> => {
		// Input variable substitution removed — return text as-is.
		return text;
	};
	...
	// Slash command tray state
	let slashTrayQuery = '';
	let slashTrayOpen = false;
	let slashTrayRef: SlashCommandTray | undefined = undefined;
	let slashTraySuppressed = false;
	// Guard to prevent form submission on the same Enter that selected a tray command
	let traySelectionJustMade = false;
	// Track if tray opens upward (active chat) or downward (initial screen)
	$: slashTrayOpenUpward = history?.currentId ? true : false;

	$: {
		slashTrayOpen = prompt.startsWith('/') && !prompt.includes('\n') && !slashTraySuppressed;
		slashTrayQuery = slashTrayOpen ? prompt.slice(1) : '';
	}
	...
	$: {
		...
		visionCapableModels =
			(currentModel?.info?.meta?.capabilities?.vision ?? true) ? [currentModelId] : [];
		fileUploadCapableModels =
			(currentModel?.info?.meta?.capabilities?.file_upload ?? true) ? [currentModelId] : [];
		webSearchCapableModels =
			(currentModel?.info?.meta?.capabilities?.web_search ?? true) ? [currentModelId] : [];
	}

	let showWebSearchButton = false;
	$: showWebSearchButton =
		webSearchCapableModels.length > 0 &&
		$config?.features?.enable_web_search &&
		($_user.role === 'admin' || $_user?.permissions?.features?.web_search);

	$: aggregateBytes = (files ?? []).reduce((sum: number, f: any) => sum + (f?.size ?? 0), 0);
	$: overAggregateCap = aggregateBytes > MAX_AGGREGATE_BYTES;
	...
	const inputFilesHandler = async (inputFiles) => {
		...
		// Pre-flight size check — runs before any upload XHR
		const rejected: string[] = [];
		const accepted: File[] = [];
		let tentativeTotal = aggregateBytes;
		for (const file of inputFiles) {
			if (file.size > MAX_PER_FILE_BYTES) {
				rejected.push(`${file.name} is larger than 20 MB`);
				continue;
			}
			if (tentativeTotal + file.size > MAX_AGGREGATE_BYTES) {
		...
		if (rejected.length) {
			toast.error(`${rejected.length} file(s) skipped:\n${rejected.join('\n')}`);
		}
				...
				<div class="">
					...
					<form
						...
						on:submit|preventDefault={() => {
							// Ignore submit if this Enter was used to select from tray
							if (traySelectionJustMade) {
								traySelectionJustMade = false;
								return;
							}
					...
					>
						...
						<div class="relative">
							<div
								...
								class="flex-1 flex flex-col relative w-full shadow-lg border {$temporaryChatEnabled
									? 'border-dashed border-gray-100 dark:border-gray-800 hover:border-gray-200 focus-within:border-gray-200 hover:dark:border-gray-700 focus-within:dark:border-gray-700'
									: ' border-gray-100/30 dark:border-gray-850/30 hover:border-gray-200 focus-within:border-gray-100 hover:dark:border-gray-800 focus-within:dark:border-gray-800'} transition px-1 bg-white/5 dark:bg-gray-500/5 backdrop-blur-sm dark:text-gray-100 {slashTrayOpen
									? slashTrayOpenUpward
										...
										{#if file.type === 'image' || (file?.content_type ?? '').startsWith('image/')}
											...
											<div class=" relative group">
												...
												<!-- Myah: size-14 (56px) matches the pre-send preview in UserMessage
													edit mode and NoteEditor/Controls; upstream's size-10 (40px) was
													too small to recognize what was attached. -->
												<Image
													src={fileUrl}
													alt=""
						...
						{#if files && files.length > 0}
							<div class="text-xs {overAggregateCap ? 'text-red-500' : 'text-gray-400'} px-3 pb-1">
								{files.length} file{files.length === 1 ? '' : 's'} · {(aggregateBytes / 1024 / 1024).toFixed(1)} / 80 MB
							</div>
						{/if}
						...
						<div class="px-2.5">
											...
											{#key $settings?.showFormattingToolbar ?? false}
												<RichTextInput
													...
													oncompositionend={(e) => {
														...
														// Clear tray suppression only on backspace/delete
														// so editing a command reopens the tray,
														// but typing normally after selection keeps it closed
														if (slashTraySuppressed && (e.key === 'Backspace' || e.key === 'Delete')) {
															slashTraySuppressed = false;
														}

															// Slash command tray keyboard handling
															if (slashTrayOpen && slashTrayRef) {
																const handled = slashTrayRef.handleKeydown(e);
																if (handled) return;
															}
							...
							<div class=" flex justify-between mt-0.5 mb-2.5 mx-0.5 max-w-full" dir="ltr">
								...
								<Tooltip content={$i18n.t('Commands')} placement="top">
									<button
										type="button"
										class="bg-transparent hover:bg-gray-100 text-gray-700 dark:text-white dark:hover:bg-gray-800 rounded-full size-8 flex justify-center items-center outline-hidden focus:outline-hidden"
										on:click={() => {
										...
										<div class=" flex items-center">
											...
											>
												<button
													...
													aria-label={uploadPending ? $i18n.t('Waiting for upload...') : $i18n.t('Send message')}
													class="{(!(prompt === '' && files.length === 0) || uploadPending) && !overAggregateCap
						...
						{#if slashTrayOpen}
							<SlashCommandTray
								bind:this={slashTrayRef}
								query={slashTrayQuery}
								openUpward={history?.currentId ? true : false}
								onSelect={(cmd) => {
									// For bypass commands with no args, send immediately
									if (cmd.bypass && !cmd.args) {
										prompt = `/${cmd.name}`;
										dispatch('submit', prompt);
									} else {
										chatInputElement?.setText(`/${cmd.name} `);
										chatInputElement?.focus?.();
									}
									// Mark that a tray selection just happened to prevent
									// the same Enter key from submitting the form
									traySelectionJustMade = true;
									// Suppress tray after selection so user can type more
									slashTraySuppressed = true;
								}}
								onClose={() => {
									// Clear slash from input to close tray
									if (slashTrayOpen) {
										chatInputElement?.setText('');
										chatInputElement?.focus?.();
									}
