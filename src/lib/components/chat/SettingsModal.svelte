<script lang="ts">
	...
	import Provider from './Settings/Provider.svelte';
	...
	const allSettings: SettingsTab[] = [
		...
		},
		{
			id: 'provider',
			title: 'Provider',
			// Base keywords that are always searchable regardless of catalog state.
			// Provider-specific names are added reactively below via $providerKeywords.
			keywords: ['provider', 'llm', 'api key', 'connect', 'key', 'disconnect']
		},
		{
			id: 'secrets',
			title: 'Secrets',
			keywords: ['api', 'key', 'env', 'secret', 'token', 'credential', 'environment']
		},
		{
			id: 'data_controls',
			title: 'Data Controls',
	...
	// ── Myah: catalog-driven provider search keywords ─────────────────────────
	// Derive provider tab keywords from the live catalog so the Settings search
	// reflects exactly the providers Hermes exposes — never a hardcoded list.
	$: {
		const catalogEntries = $catalog ? Object.values($catalog) : [];
		const catalogKeywords = catalogEntries.flatMap((e) => {
			const words: string[] = [];
			if (e.id) words.push(e.id.toLowerCase());
			if (e.name) words.push(e.name.toLowerCase());
			return words;
		});
		const providerTab = allSettings.find((t) => t.id === 'provider');
		if (providerTab) {
			const base = ['provider', 'llm', 'api key', 'connect', 'key', 'disconnect'];
			providerTab.keywords = [...new Set([...base, ...catalogKeywords])];
		}
	}
	// ──────────────────────────────────────────────────────────────────────────
					...
					{#each filteredSettings as tabId (tabId)}
						...
						{:else if tabId === 'provider'}
							...
							>
								...
								<div class=" self-center">{$i18n.t('Provider')}</div>
							</button>
						{:else if tabId === 'secrets'}
							...
							>
								...
								<div class=" self-center">{$i18n.t('Secrets')}</div>
							</button>
